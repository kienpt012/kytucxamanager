...
namespace TECH.Areas.Admin.Controllers
{
    public class ChiTietHoaDonController : BaseController
    {
        ...
        private readonly IUnitOfWork _unitOfWork; // Nếu dùng UnitOfWork
        ...
        public IActionResult Index(int mahoadon, int maphong)
        {
            ...
            if (mahoadon > 0 && maphong > 0)
            {
                ...
                if (phong != null)
                {
                    ...
                    lstChiTietHoaDonIndexModelViews.SoDienCu = (int?)phong.SoDienCu;
                    lstChiTietHoaDonIndexModelViews.SoNuocCu = (int?)phong.SoNuocCu;
                }
        ...
        // Phạt tiền khi phạm lỗi
        [HttpPost]
        public JsonResult UpdateChiSoPhong(int maPhong, int soDienMoi, int soNuocMoi)
        {
            if (maPhong > 0)
            {
                try
                {
                    // Sử dụng repository thay cho DbContext
                    var phong = _phongRepository.FindById(maPhong); // Repository của Phòng
                    if (phong != null)
                    {
                        // Cập nhật chỉ số điện và nước
                        phong.SoDienCu = soDienMoi;
                        phong.SoNuocCu = soNuocMoi;

                        // Lưu thay đổi
                        _phongRepository.Update(phong);
                        _unitOfWork.Commit(); // Lưu vào database thông qua UnitOfWork
